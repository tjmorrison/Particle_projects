 ...
 *              beacon detection on D3. Logs detection data every 15 minutes, publishes
...
void loop() {
    ...
    // Sleep until 15-min interval ends OR wake on PIR/Beacon trigger
    // Each GPIO trigger wakes the device and increments counters
    ...
    // Check for detections - each GPIO wake increments the counter
    ...
    if (digitalRead(Pin_Beacon) == HIGH){
      ...
      SendBeaconAck();
      Log.info("Beacon detected, count for interval: %d", Beacon_cnt);
    }
